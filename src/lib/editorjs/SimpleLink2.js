...
export default class LinkTool {
  ...
  showLinkPreview({ image, title, description }) {
    this.nodes.container.appendChild(this.nodes.linkContent);
    console.log('Link Preview', image)
    if (image) {
        console.log('Here');
      this.nodes.linkImage.style.backgroundImage = 'url(' + image.url + ')';
      this.nodes.linkImage.style.height = '100px';
      this.nodes.linkImage.style.backgroundSize = 'contain';
    ...
    if (description) {
      this.nodes.linkDescription.textContent = description.slice(0,100);
   ...
   */
  async fetchFromOpengraph(url) {
    let d = {};
    
    d.success = 1;
    d.meta = {};
    const {body } = await ajax.get({url: 'https://opengraph.io/api/1.1/site/' + encodeURIComponent(url) + '?app_id=ebd53bb7-5bfd-4a22-b3ee-0cbd6710dc77'});
    console.log('URL', 'https://opengraph.io/api/1.1/site/' + encodeURIComponent(url) + '?app_id=ebd53bb7-5bfd-4a22-b3ee-0cbd6710dc77');
    if (body.openGraph.error){
        d.meta.title = body.hybridGraph.title;
        d.meta.description = body.hybridGraph.description;
        if(body.hybridGraph.image) {d.meta.image = {}; d.meta.image.url = body.hybridGraph.image};
    }
    else d.meta = body.openGraph;
    // d.meta = body.openGraph;
  ...
  async fetchLinkData(url) {
    ...
    try {
      const body = await this.fetchFromOpengraph(url);
